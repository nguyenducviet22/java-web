...
public class Customer_Inter_DAO implements Interface_DAO<Customer> {
	...
	public ArrayList<Customer> selectAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				String linkAvatar = rs.getString("link_avatar");

				Customer customer = new Customer(customerCode, username, password, fullName, gender, address,
						receivedAddress, dateOfBirth, phoneNumber, email, verifyCode, validPeriod, verifyState,
						linkAvatar);
				result.add(customer);
			}
		...
		try {
			...
			while (rs.next()) {
				...
				result = new Customer(customerCode, username, password, fullName, gender, address, receivedAddress,
						dateOfBirth, phoneNumber, email, verifyCode, validPeriod, verifyState, linkAvatar);
	...
	// lesson 41 update avatar
	public int updateAvatar(Customer t) {
		int result = 0;
		try {
			// #1 create connection with CSDL
			Connection con = JDBCUtil.getConnection();
			...
			// #2 create SQL command and PreparedStatement
			String sql = "update customer " + " set " + " link_avatar = ?" + " where customer_code = ?";

			PreparedStatement pst = con.prepareStatement(sql);
			pst.setString(1, t.getLinkAvatar());
			pst.setString(2, t.getCustomerCode());
